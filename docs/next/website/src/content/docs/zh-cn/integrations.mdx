...
Herdr 对 Linux/macOS 上的集成配置和所有平台的新文件使用原子保存；Windows 上的现有文件先备份再原地更新，以保留权限。如果 Windows 提示备份仍存在，请在重试前关闭智能体并检查配置，必要时仅将完整 `.herdr-backup` 的内容写回现有文件，确认配置正确后再删除备份，切勿从未完成的 `.herdr-backup.pending` 文件恢复。
