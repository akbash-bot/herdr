...
Shared agent configuration is written to a temporary file and replaced only after the complete write succeeds. Herdr preserves file permissions and follows symlinks. Config files with multiple hard links are rejected before installation or removal changes hook files; use a separate file or a symlink before retrying. This protects each config file, not an entire multi-file installation from partial completion.
