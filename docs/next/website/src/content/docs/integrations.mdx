...
Herdr saves integration configs atomically on Linux/macOS and for new files; existing Windows files keep their permissions through a backup-first, in-place update. If Windows reports a retained backup, close the agent and inspect the config before retrying: restore only completed `.herdr-backup` contents into the existing file if needed, verify the config, then remove the backup—never restore from an unfinished `.herdr-backup.pending` file.
