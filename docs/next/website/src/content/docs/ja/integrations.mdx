...
共有されるエージェント設定は一時ファイルに書き込まれ、書き込みがすべて成功した後に置き換えられます。Herdr はファイルのアクセス権を保持し、シンボリックリンクをたどります。複数のハードリンクを持つ設定ファイルは、インストールやアンインストールでフックファイルを変更する前に拒否されます。独立したファイルまたはシンボリックリンクに変更してから再試行してください。この保護は設定ファイル単位であり、複数ファイルにまたがる操作全体のロールバックを保証するものではありません。
