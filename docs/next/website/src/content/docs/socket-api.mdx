...
Error responses also echo the request's `id`, including when the method or
parameters are invalid. If the request is not valid JSON or has no unambiguous
top-level string `id`, the error response uses an empty `id`.
